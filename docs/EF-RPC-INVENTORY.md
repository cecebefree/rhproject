...
| `set_handle` | `supabase/functions/set_handle/index.ts` | 205 | DB tests green (062: 25/25, 062a: 17/17); HTTP matrix + deploy pending | set_handle EF — authority-enforced handle assignment (3-20 chars, no whitespace, lowercase), per-tenant uniqueness, global reserved deny-list per contract v1, audit via handle_changes (NO RLS) |
