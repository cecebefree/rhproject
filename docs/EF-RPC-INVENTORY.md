...
| `set_handle` | `supabase/functions/set_handle/index.ts` | 205 | COMPLETE — verified 2026-07-25 (commit 7354510, pgTAP 264/264, 6/6 HTTP smoke incl. cross-tenant-admin denial) | set_handle EF — authority-enforced handle assignment (3-20 chars, no whitespace), per-tenant uniqueness, blocklist (admin|office|system|support|root|api|redhouse+variants), audit via handle_changes (NO RLS) |
