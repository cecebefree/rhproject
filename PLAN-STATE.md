...
| 42 | E2E demo + Cece sign-off — terminal human gate | PENDING | gated on 31-36, 46, 48 (Row 41 adversarial RLS CLOSED 2026-08-07, Gate C satisfied); 31-36 DONE-LOCAL pending Cece scope ruling; Row 48 (leads-read) PENDING Backend/QA — PREREQUISITE (admin must view submitted leads in E2E) |
...
**BOARD-SYNC (2026-08-09):** Row 45 corrected from UNALLOCATED to DONE (Acceptance Checklist). Canon file existed at `docs/canon/row-45-acceptance-checklist.md` since 2026-08-03; board table marker was stale. Scoreboard updated 42→43.
...
6. **Row 42 (E2E demo)** - BLOCKED ON rows 31-36 (DONE-LOCAL, pending Cece scope ruling) + Row 46 seed re-tag (DONE, f192dc7) + Row 48 (leads-read, PENDING, Backend/QA — PREREQUISITE); Row 41 adversarial RLS CLOSED (fix 091 verified 2026-08-07, Gate C satisfied); Row 44 (leads-write) DONE (c5941d0)
...
- **Observation (data follow-up, R4/Backend):** post-091 seeded courses carry `tenant_id = NULL` because 091's backfill `UPDATE courses…FROM profiles` ran at migration-time on empty tables (seed inserts via `scripts/seed-users.sh` arrive AFTER reset). Consequently a *Redhouse-scoped authenticated* caller would also resolve `tenant_id = NULL` (no published course returned) — a functional/data gap, NOT a security regression (no rows leak; 41a-3 passes). Recommended fix: re-tag seeded Redhouse courses to `e97e5c3a-1234-4321-abcd-000000000001` (the Redhouse `tenant_lms.id` per seed.sql:21 / seed-users.sh:12 — `courses.tenant_id` FK references `tenant_lms.id`) post-seed — NOT `00000000-0000-0000-0000-000000000001`, which is not a valid tenant_lms row (stale v4.4 carry-in), or re-run 091's backfill after seeding. Out of scope for 41a-3 security closure.
