...
- **Doctrine — profiles.role is the authoritative role source for RLS; JWT `app_metadata.role` alone does not create a role identity.** Tests asserting role-gated behavior MUST set the DB fixture row (`UPDATE public.profiles SET role = ...`), not just the JWT claims. The `courses_no_core_outside` RESTRICTIVE policy reads `profiles.role`; a role set only in `app_metadata` is invisible to it (see docs/governance/item-33-test-envelope-and-fixture-doctrine.md).

- **Shell guard:** `rm -f` / `rm -rf` are blocked under `/Users`; plain `rm <file>` on a specific absolute path works. Use it for targeted deletions only.
