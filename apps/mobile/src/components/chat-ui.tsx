...
export function SendIndicator({ state, onRetry }: { state: SendState; onRetry?: () => void }) {
  ...
  if (state === 'failed') {
    return (
      <TouchableOpacity onPress={onRetry} style={{ flexDirection: 'row', alignItems: 'center', gap: 4 }}>
...
export function ReconnectingBanner({ visible }: { visible: boolean }) {
  ...
  return (
    <View style={{ backgroundColor: '#f39c12', paddingVertical: 6, paddingHorizontal: 16, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8 }}>
...
export function EmptyGroups() {
  return (
    <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center', padding: 40 }}>
      ...
      <Text style={{ fontSize: 18, fontWeight: '600', color: '#333', textAlign: 'center' }}>No groups yet</Text>
...
export function EmptyChat() {
  return (
    <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center', padding: 40 }}>
      ...
      <Text style={{ fontSize: 18, fontWeight: '600', color: '#333', textAlign: 'center' }}>No messages yet</Text>
      <Text style={{ fontSize: 14, color: '#666', textAlign: 'center', marginTop: 8 }}>
...
/* G4 -- Group info view */
export function GroupInfoView({ group, onClose }: {
  group: { name: string; category: string; memberCount: number; isLead: boolean; createdAt: string };
  onClose: () => void;
}) {
  return (
    <View style={{ position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0,0,0,0.4)', justifyContent: 'flex-end' }}>
      <View style={{ backgroundColor: 'white', borderTopLeftRadius: 16, borderTopRightRadius: 16, padding: 24, maxHeight: '70%' }}>
        ...
        <View style={{ gap: 12 }}>
          ...
          <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
            ...
            <Text style={{ fontWeight: '500', color: group.isLead ? '#c0392b' : '#666' }}>{group.isLead ? 'Yes' : 'No'}</Text>
          </View>
          <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
