...
export default function TeacherDashboardPage() {
  ...
  async function loadData() {
    ...
    if (coursesData && coursesData.length > 0) {
      ...
      const mapped = (studentsData ?? []).map((row: Record<string, unknown>) => {
        const profiles = row.profiles as Record<string, unknown> | null;
        const programs = row.programs as Record<string, unknown> | null;
        return {
          id: row.student_id as string,
          name: (profiles?.name as string) ?? 'Unknown',
          email: (profiles?.email as string) ?? '',
          grade: (profiles?.grade as string) ?? '',
          course_title: (programs?.title as string) ?? '',
        };
      });
    ...
    <AdminLayout activeDesk="school-desk">
      ...
      <div className="grid grid-cols-3 gap-4 mt-6">
        ...
        >
          ...
          <p className="text-2xl font-semibold mt-1" style={{ color: '#1A242B' }}>
            {programs.length}
          </p>
        </div>
        <div
      ...
      >
        ...
        {students.length === 0 ? (
          <p className="text-sm" style={{ color: '#54626C' }}>
            No students enrolled in your programs
          </p>
        ) : (
          <div className="space-y-4">
            {Object.entries(studentsByCourse).map(([courseTitle, courseStudents]) => (
              <div key={courseTitle}>
                <h3 className="text-xs font-semibold mb-2" style={{ color: '#54626C' }}>
                  {courseTitle}
                </h3>
