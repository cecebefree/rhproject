...
function getStatusStyle(status: string) {
  switch (status) {
    case 'overdue':
      return { backgroundColor: 'rgba(200,40,30,0.1)', color: '#C8281E' };
    case 'issued':
      return { backgroundColor: '#dbdad7', color: '#54626C' };
    case 'draft':
      return { backgroundColor: '#dbdad7', color: '#54626C' };
    case 'paid':
      return { backgroundColor: 'rgba(39,57,70,0.1)', color: '#273946' };
    default:
      return { backgroundColor: '#dbdad7', color: '#54626C' };
  }
}
...
export function FamilyAccountsDefault() {
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      const familyIds = [
        ...new Set(
          (invoicesData || []).map((i: Record<string, unknown>) => i.family_account_id as string)
        ),
      ];
      ...
      for (const f of (familiesData || []) as Record<string, unknown>[]) {
        familyMap.set(f.id as string, {
          family_name: (f.family_name as string) || 'Unknown',
          family_code: (f.family_code as string) || '—',
        });
      }
              ...
              <thead>
                ...
                >
                  {['Family / Account', 'Inv. Date', 'Due Date', 'Amount', 'Status'].map((h) => (
                    <th
                      key={h}
                      className={`py-3 px-6 font-semibold ${h === 'Amount' ? 'text-right' : ''}`}
                      style={{
                        fontSize: '11px',
                        fontWeight: 600,
                ...
                {invoices.map((inv) => {
                  ...
                  return (
                    <tr
                      ...
                      onMouseEnter={(e) => {
                        (e.currentTarget as HTMLTableRowElement).style.backgroundColor = '#f4f3f0';
                      }}
                      onMouseLeave={(e) => {
                    ...
                    >
                      <td className="py-4 px-6">
                        <div className="font-medium" style={{ color: '#273946', fontSize: '14px' }}>
                          {inv.family_name}
                        </div>
                        <div
                          className="mt-0.5"
                      ...
                      <td
                        className="py-4 px-6"
                        style={{
                          color: inv.status === 'overdue' ? '#C8281E' : '#1A242B',
      ...
      >
        ...
        >
          ...
          <div className="flex items-center gap-4">
            ...
            >
              Total Records: {totalRecords} | Invoiced:{' '}
              {totalRecords > 0 ? Math.round((invoicedCount / totalRecords) * 100) : 0}%, Paid:{' '}
              {totalRecords > 0 ? Math.round((paidCount / totalRecords) * 100) : 0}%, Overdue:{' '}
              {totalRecords > 0 ? Math.round((overdueCount / totalRecords) * 100) : 0}%, Draft:{' '}
              {totalRecords > 0 ? Math.round((draftCount / totalRecords) * 100) : 0}%
            </p>
            <button type="button" className="hover:text-[#273946]" style={{ color: '#54626C' }}>
                ...
                <p style={{ fontSize: '14px', lineHeight: '20px', color: '#1A242B' }}>
                  {inv.status === 'overdue' ? (
                    <>
                      ...
                      account flagged as{' '}
                      <span className="font-semibold" style={{ color: '#C8281E' }}>
                        Overdue
                      </span>
                    </>
                  ) : inv.status === 'paid' ? (
