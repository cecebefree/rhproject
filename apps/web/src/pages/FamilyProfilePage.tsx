...
import { type FamilyData, fetchFamilyById } from '../lib/crmClient';
...
export default function FamilyProfilePage() {
  ...
  if (error || !family)
    return (
      <AdminLayout activeDesk="crm">
        <div className="p-8 text-center text-red-600">{error || 'Not found'}</div>
      </AdminLayout>
    );
  ...
  const contactActivities = [
    {
      ...
      label: 'ACCOUNT NOTE',
      description: 'All paperwork verified and up to date for the current academic year.',
      timestamp: '15 May 2024',
    },
    {
      type: 'general' as const,
      label: 'ENROLLMENT CONFIRMED',
      description: 'Both students enrolled for Spring 2024 term. Tuition paid in full.',
      timestamp: '01 Sep 2023',
    },
    {
      type: 'medical' as const,
      label: 'MEDICAL RECORD UPDATE',
      description: 'Emergency contact details updated by Sarah Montgomery.',
  ...
  return (
    <AdminLayout activeDesk="crm">
        ...
        <div className="flex items-center gap-6">
          ...
          <div>
            ...
            <p className="text-sm" style={{ color: '#54626C' }}>
              Family Account • ID: {family.family.id}
            </p>
          </div>
        </div>
        <button type="button"
      ...
      <section>
        ...
        <div
          className="rounded p-8 grid grid-cols-1 md:grid-cols-5 gap-8"
          style={{ backgroundColor: '#ffffff', border: '1px solid rgba(195,199,204,0.2)' }}
        >
          {[
            {
              label: 'Enrollment Date',
              value: new Date(family.family.created_at).toLocaleDateString(),
            },
            ...
            <div key={item.label}>
              ...
              <p className="text-base" style={{ color: '#1A242B' }}>
                {item.value}
              </p>
            </div>
          ))}
                ...
                {familyData.adults.map((adult, i) => (
                  ...
                  >
                    <td className="py-4 text-sm" style={{ color: '#1A242B' }}>
                      <Link
                        to={`/service/crm/family/${familyId}/adult/${adult.id}`}
                    ...
                    <td className="py-4 text-sm" style={{ color: '#54626C' }}>
                      {adult.role}
                    </td>
                    <td className="py-4">
                      <span
              ...
              <tbody>
                {familyData.students.map((student, i) => (
                  <tr
                    ...
                    style={{
                      borderBottom:
                        i < familyData.students.length - 1
                          ? '1px solid rgba(195,199,204,0.1)'
                          : undefined,
                    }}
                  >
                    <td className="py-4 text-sm" style={{ color: '#1A242B' }}>
                      <Link
                        to={`/service/crm/family/${familyId}/student/${student.id}`}
                        className="hover:underline"
                        style={{ color: '#273946' }}
                      >
                        {student.name}
        ...
        >
          ...
          >
            ...
            <div className="flex items-center gap-4">
              ...
              >
                Total Invoices: {familyInvoices.length} | Paid:{' '}
                {familyInvoices.filter((i) => i.status === 'paid').length} | Pending:{' '}
                {familyInvoices.filter((i) => i.status !== 'paid').length}
              </p>
              ...
              familyInvoices.slice(0, 5).map((invoice, i) => (
                <div key={invoice.id} className="flex gap-4 relative">
                  ...
                  <div
                    ...
                    style={{
                      backgroundColor:
                        invoice.status === 'paid' ? 'rgba(39,57,70,0.1)' : 'rgba(232,160,32,0.1)',
                      border:
                        invoice.status === 'paid'
                  ...
                  <div>
                    ...
                    <p className="text-xs mt-1" style={{ color: '#54626C' }}>
                      ...
                      {invoice.amount?.toFixed(2) || '0.00'} • {invoice.status}
                    </p>
                  </div>
                </div>
              ))
              ...
              <thead>
                <tr style={{ backgroundColor: 'rgba(244,243,240,0.3)' }}>
                  {['Date', 'Amount', 'Method', 'Status'].map((h) => (
                    <th
                      key={h}
                      className="px-8 pb-4 uppercase border-b"
                      style={{
                        fontSize: '11px',
                        letterSpacing: '0.12em',
                        fontWeight: 600,
                        color: '#54626C',
                        borderColor: 'rgba(195,199,204,0.2)',
                      }}
                    >
                      {h}
                    </th>
                ...
                {familyPayments.length === 0 ? (
                  ...
                  familyPayments.map((payment, i) => (
                    ...
                    >
                      ...
                      <td className="px-8 py-4 text-sm" style={{ color: '#1A242B' }}>
                      ...
                      <td
                        ...
                        style={{ color: payment.status === 'completed' ? '#1A242B' : '#E8A020' }}
                      >
                        {payment.status}
                      </td>
                    </tr>
      ...
      <section>
        <div
          className="rounded-xl overflow-hidden"
          style={{ border: '1px solid rgba(39,57,70,0.1)', backgroundColor: '#ffffff' }}
        >
          <div
            className="px-8 py-6 flex justify-between items-center"
            style={{ borderBottom: '1px solid rgba(195,199,204,0.2)' }}
          ...
          <div className="p-8 grid grid-cols-1 md:grid-cols-3 gap-8">
            <div className="flex flex-col gap-3">
              ...
              >
                {family.family.family_code || family.family.id?.substring(0, 8) || 'N/A'}
              </div>
              <p className="text-xs" style={{ color: '#54626C' }}>
                Adults in Account: {familyData.adults.length}
              </p>
            </div>
            <div className="flex flex-col gap-3">
              <span
            ...
            <div className="flex flex-col gap-3">
              ...
              <div className="space-y-2">
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-xs font-medium" style={{ color: '#1A242B' }}>
                    Paid
                  </span>
                </div>
                <div className="flex justify-between items-center">
          ...
          <div
            className="px-8 py-6 flex flex-wrap gap-4"
            style={{ borderTop: '1px solid rgba(39,57,70,0.05)' }}
          >
            <button type="button"
              className="px-6 py-2.5 rounded text-xs font-semibold uppercase tracking-wider cursor-pointer"
              style={{
                backgroundColor: '#273946',
                color: '#ffffff',
                fontSize: '11px',
            ...
            <button type="button"
              className="px-6 py-2.5 rounded text-xs font-semibold uppercase tracking-wider flex items-center gap-2 cursor-pointer"
              style={{
                border: '1px solid rgba(39,57,70,0.2)',
                color: '#273946',
                fontSize: '11px',
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 space-y-6">
            <div className="grid grid-cols-2 gap-6">
              <div
                ...
                style={{
                  ...
                  border: '1px solid rgba(195,199,204,0.1)',
                }}
              >
                <p
                  className="mb-1 uppercase"
                ...
                <p className="text-sm" style={{ color: '#1A242B' }}>
                  ID: {family.family.id?.substring(0, 8) || 'N/A'} • Good Standing
                </p>
              </div>
              <div
                className="p-4 rounded"
            ...
            >
              ...
              <p className="text-xs" style={{ color: '#54626C' }}>
                Account is currently in good standing. All administrative paperwork for the current
                academic year is complete and verified by the Front Desk. No outstanding balances or
                pending disciplinary actions recorded.
              </p>
            </div>
          </div>
        </div>
