...
const DESKS = [
  {
    ...
    label: 'Super Admin',
    icon: 'dashboard',
    desc: 'Super admin — access all desks',
    href: '/service/service-desk',
  },
  {
    key: 'front-desk',
    label: 'Front Desk',
...
export default function ServiceDeskLogin() {
  ...
  const { isAuthenticated, login, selectDesk } = useServiceDeskAuth();
  ...
  const handleSelectDesk = (href: string, deskKey: string) => {
    selectDesk(deskKey);
    navigate(href);
  };
        ...
        <div className="w-full max-w-lg">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {DESKS.map((desk) => (
              <button type="button"
                key={desk.key}
                onClick={() => handleSelectDesk(desk.href, desk.key)}
              ...
              >
                <span
                  ...
                  style={{ fontSize: '32px', color: '#E8A020' }}
                >
                  {desk.icon}
                </span>
                <h3
                  className="text-sm font-semibold uppercase"
                  style={{ fontSize: '12px', letterSpacing: '0.1em', color: '#1A242B' }}
                >
                  {desk.label}
                </h3>
      ...
      <div className="w-full max-w-sm">
        ...
        <form onSubmit={handleLogin} className="space-y-4">
          <div>
            ...
            />
            {error && (
              <p className="text-xs mt-2" style={{ color: '#C8281E' }}>
                {error}
              </p>
            )}
          </div>
