...
import { type HttpMethod, OfflineQueueManager } from '../services/offlineQueue';
import { type OptimisticUpdate, OptimisticUpdateManager } from '../services/optimisticUpdate';
import {
  type ConflictItem,
  type ConflictStrategy,
  detectConflict,
  resolveConflict,
} from '../services/realtimeConflict';
...
export function useOptimisticMutation<T extends Record<string, unknown>>(
  ...
  const mutate = useCallback(
    async (pendingData: Partial<T>, method: HttpMethod = 'UPDATE'): Promise<T> => {
      setIsPending(true);
      setError(null);
  ...
  const resolveConflictManually = useCallback(
    (conflict: ConflictItem, resolutionStrategy: ConflictStrategy): T => {
