...
export function useExport({ tenantId, userId, deskId }: UseExportOptions) {
  ...
  const exportData = useCallback(
    async (
      ...
      try {
        ...
        const result =
          format === 'csv' ? await exportToCSV(exportOptions) : await exportToPDF(exportOptions);
  ...
  const loadTemplates = useCallback(
    async (entityType?: ExportEntityType) => {
  ...
  const createTemplate = useCallback(
    async (template: Omit<ReportTemplate, 'id' | 'created_at' | 'updated_at' | 'tenant_id'>) => {
  ...
  const createScheduledReport_ = useCallback(
    async (
  ...
  const updateScheduledReport_ = useCallback(
    async (reportId: string, updates: Partial<ScheduledReport>) => {
      if (!hasPermission('settings.edit')) {
  ...
  const deleteScheduledReport_ = useCallback(
