...
export function useContactNotes({
  ...
  const createNote = useCallback(
    async (content: string, mentionUserIds?: string[]): Promise<ContactNote | null> => {
  ...
  const addAttachment = useCallback(
    async (noteId: string, file: File): Promise<ContactNoteAttachment | null> => {
      setError(null);
    ...
    setNotes((prev) =>
      prev.map((n) => ({
        ...
        attachments: (n.attachments || []).filter(
          (a) => (a as ContactNoteAttachment).id !== attachmentId
        ),
      }))
    );
