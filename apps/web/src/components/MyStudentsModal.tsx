...
export default function MyStudentsModal({
  isOpen,
  onClose,
  subject,
  className,
  onEditReportCard,
}: MyStudentsModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div
        ...
        onClick={onClose}
        onKeyPress={(e) => { if (e.key === 'Escape') onClose(); }}
      />

      {/* Modal */}
      <div
        className="relative bg-white border rounded-lg w-full max-w-4xl max-h-[85vh] flex flex-col mx-4"
      ...
      >
        ...
        >
          <div>
            ...
            <p className="mt-1" style={{ fontSize: '14px', color: '#54626C' }}>
              Class: {subject} - {className}
            </p>
          </div>
          <button type="button"
            onClick={onClose}
            className="w-10 h-10 flex items-center justify-center rounded hover:bg-surface-container-low transition-colors group"
          >
            <span className="material-symbols-outlined text-text-muted group-hover:text-brand-navy">
              close
            </span>
          </button>
        </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr style={{ borderBottom: '1px solid rgba(39,57,70,0.1)' }}>
                ...
                <th
                  className="pb-4 uppercase text-right"
                  style={{
                    fontSize: '11px',
                    letterSpacing: '0.12em',
                    fontWeight: 600,
            ...
            <tbody>
              {visibleStudents.map((student) => (
                <tr
                  key={student.name}
                  className="hover:bg-surface-cream transition-colors"
                ...
                >
                  ...
                  <td
                    ...
                    style={{ fontSize: '13px', color: '#54626C' }}
                  >
                    {student.grade}
                  </td>
                  <td className="py-3 px-4 align-middle">
                    <span
                      ...
                      style={{
                        backgroundColor:
                          student.enrollmentStatus === 'At Risk'
                            ? 'rgba(200,40,30,0.05)'
                            : 'rgba(39,57,70,0.05)',
        ...
        >
          <span style={{ fontSize: '12px', color: '#54626C' }}>
            Showing {visibleStudents.length} of {MOCK_STUDENTS.length} students
          </span>
