...
export function AdminLayout({ children, activeDesk }: AdminLayoutProps) {
  ...
  return (
    <div
      ...
      style={{ fontFamily: '"Source Sans 3", sans-serif', color: '#1A242B' }}
    >
      {/* Sidebar — Desktop */}
      <nav
      ...
      >
            ...
            <>
              ...
              <li>
                <Link
                  ...
                  style={{
                    borderLeft:
                      activeDesk === 'crm' ? '4px solid #E8A020' : '4px solid transparent',
        ...
        {/* Footer Nav */}
        <ul
          className="px-4 py-6 mt-auto space-y-2"
        ...
        >
          {FOOTER_ITEMS.map((item) => (
            <li key={item.label}>
              <Link
                to={item.href}
                className="flex items-center gap-3 px-4 py-3 rounded text-white/70 hover:text-white hover:bg-white/5 transition-all duration-200 no-underline"
              >
                <span className="material-symbols-outlined" style={{ fontSize: '20px' }}>
                  {item.icon}
                </span>
      ...
      {sidebarOpen && (
        <div className="fixed inset-0 z-50 md:hidden">
          <div className="absolute inset-0 bg-black/50" onClick={() => setSidebarOpen(false)} onKeyPress={(e) => { if (e.key === 'Escape') setSidebarOpen(false); }} />
          <nav
      ...
      {/* Main Content */}
      <div
        className="flex-1 flex flex-col min-w-0 md:ml-64 h-full"
        style={{ backgroundColor: '#F8F7F4' }}
      >
        {/* Top Bar */}
        <header
          className="border-b w-full h-16 flex justify-between items-center px-4 md:px-16 z-40 sticky top-0"
          style={{ backgroundColor: '#faf9f6', borderColor: 'rgba(195,199,204,0.3)' }}
        >
          <div className="flex items-center gap-4">
            <button type="button" className="md:hidden text-gray-500" onClick={() => setSidebarOpen(true)}>
              <span className="material-symbols-outlined">menu</span>
            </button>
            <div
              className="hidden md:flex items-center rounded px-3 py-1.5"
            ...
            <div className="flex items-center gap-4" style={{ color: '#112430' }}>
              ...
              >
                ...
                <span
                  ...
                  style={{ backgroundColor: '#C8281E', border: '1px solid #faf9f6' }}
                />
              </button>
              <button type="button" className="hover:bg-gray-100 transition-colors duration-200 p-2 rounded-full cursor-pointer">
                <span className="material-symbols-outlined">help</span>
              </button>
            </div>
            <div
              className="hidden sm:flex items-center gap-3 pl-4 cursor-pointer group"
              style={{ borderLeft: '1px solid rgba(195,199,204,0.3)' }}
            >
              <div className="text-right">
                <p
                  className="text-xs font-semibold uppercase"
                  style={{ fontSize: '11px', letterSpacing: '0.12em', color: '#1A242B' }}
                >
                  Eleanor V.
                </p>
              ...
              <div
                ...
                style={{ border: '1px solid rgba(195,199,204,0.5)' }}
              >
                <div
                  className="w-full h-full flex items-center justify-center text-sm font-medium"
                  style={{ backgroundColor: '#e9e8e5', color: '#273946' }}
                >
                  EV
                </div>
              </div>
