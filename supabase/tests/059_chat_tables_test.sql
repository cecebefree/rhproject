...
INSERT INTO school_desk.conversations (id, tenant_id, created_by)
VALUES ('cccccccc-cccc-cccc-cccc-cccccccccccc', 'aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa', '11111111-1111-1111-1111-111111111111');
INSERT INTO school_desk.conversation_members (conversation_id, profile_id)
VALUES ('cccccccc-cccc-cccc-cccc-cccccccccccc', '11111111-1111-1111-1111-111111111111');

-- 1-5. Structure: tables exist
SELECT ok( EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema='school_desk' AND table_name='conversations'), 't1 conversations exists');
SELECT ok( EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema='school_desk' AND table_name='conversation_members'), 't2 conversation_members exists');
SELECT ok( EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema='school_desk' AND table_name='messages'), 't3 messages exists');
...
SELECT throws_ok(
  $$INSERT INTO school_desk.conversations (id, tenant_id, created_by) VALUES ('aeaeaeae-aeae-aeae-aeae-aeaeaeaeaeae', NULL, '11111111-1111-1111-1111-111111111111')$$,
  '23502' );
...
SELECT is( (SELECT count(*)::int FROM school_desk.messages WHERE id='eeeeeeee-eeee-eeee-eeee-eeeeeeeeeeee' AND deleted_at IS NOT NULL), 1, 't16b soft-deleted row remains with deleted_at set');
